...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %s %d",S_Name[i],R_Name[i],&hit_locus[i],T_Name[i],&hit_length[i])!=EOF)
    {
        i++;
    }
    fclose(namef);
...
{
     int i,j,k,m,n;
     int num_hits,*ctg_index,*hit_index;
     FILE *namef;
     ...
     {
        int idt = readIndex[i];
        fprintf(namef,"%s %s %d %s %d\n",S_Name[i],R_Name[idt],hit_locus[idt],T_Name[idt],hit_length[idt]);
     }
     fclose(namef);
}
