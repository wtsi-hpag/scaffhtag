...
static int *hit_locus,*hit_index,*hit_rcdex,*hit_length,*ctg_index;
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %s %s %s %s %s %s %s %d",readname,readname,readname,S_Name[i],readname,readname,readname,readname,readname,&hit_length[i])!=EOF)
    {
        int idt;
//    printf("%s %s %d %d %d %d\n",S_Name[i],readname,hit_locus[i],hit_length[i],ctg_index[i],hit_rcdex[i]);
        i++;
    }
    fclose(namef);
