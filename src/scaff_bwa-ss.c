...
int main(int argc, char **argv)
{
    ...
    char barcode[100],barcode1[100],barcode2[100],barcoderc[100];
    char line[2000000]={0},bscore[20],score1[20],score2[20],temp[1000],readname[Max_N_NameBase];
...
/*
           strncpy(barcode,base0,13);
           strncpy(barcode1,base0,13);
           st = strrchr(base0,'-');
           strcpy(barcode2,st+1);
           for(j=bar_len;j>0;j--)
           {
                if(barcode2[j-1] == 'A')
                  barcoderc[13-j] = 'T';
                else if(barcode2[j-1] == 'C')
                  barcoderc[13-j] = 'G';
                else if(barcode2[j-1] == 'G')
                  barcoderc[13-j] = 'C';
                else if(barcode2[j-1] == 'T')
                  barcoderc[13-j] = 'A';
                else
                  barcoderc[13-j] = 'N';
           }
	   if(IMOD == 1)
             strcat(barcode,barcoderc);
	   else
             strcat(barcode,barcode2);
//           printf("align00 %s %s %s %s\n",barcode,barcode1,barcode2,barcoderc);
                        */
//           printf("align00 %s %s %s %s\n",barcode,bcode1,bcode2,bcode3);
         }
         else if(i==1)
         {
           memset(base1,'\0',500);
