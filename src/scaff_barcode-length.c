...
int main(int argc, char **argv)
{
    ...
    if(argc < 2)
    {
      printf("Usage: %s [-len 20000] [-reads 5] <input_barcode_file> <barcode_length_file>\n",argv[0]);
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %d %d",S_Name[i],readname,&hit_locus[i],&hit_score[i],&hit_length[i])!=EOF)
    {
        int idt;
        st = readname;
...
//    printf("%s %s %d %d %d %d\n",S_Name[i],readname,hit_locus[i],hit_length[i],ctg_index[i],hit_score[i]);
        i++;
    }
    fclose(namef);
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
        ...
        {
          if((strcmp(S_Name[i],S_Name[j])==0)&&(ctg_index[i]==ctg_index[j]))
          {
            j++;
          }
          else
        ...
        num_hits = j-i;
        if((num_hits>=barreads)) 
        {
          int n_coverbase = 0;
          int kk,n_ave = 0;
          int num_blocks = 0;
          int stopflag2,num_hit2;
          int num_linkbase,len,len2,mscore;
	  ...
	  kk = j-i;
          ...
          for(n=i;n<j;n++)
          {
             ...
             {
                if(((hit_locus[m]-hit_locus[m-1])<=max_len)&&(m<(j)))
                {
//               printf("%s %d %d %d\n",S_Name[m],ctg_index[m],hit_locus[m-1],hit_locus[m]); 
                  m++;
                }
                else
                  stopflag2=1;
             ...
             {
               float rate;
               int len = hit_locus[m-1]-hit_locus[n];

               mscore = 0;
               for(k=n;k<m;k++)
                  mscore = hit_score[k]+mscore;
               mscore = mscore/num_hit2;
               rate = num_hit2;
               if(len <= 0)
                 len = 1; 
               rate = len/rate;
               fprintf(namef,"%s tarseq_%d %d %d %d %d %d %f %d\n",S_Name[n],ctg_index[n],hit_locus[m-1]-hit_locus[n],hit_length[n],hit_locus[n],hit_locus[m-1],num_hit2,rate,mscore); 
             }
             n=m-1;
          }
