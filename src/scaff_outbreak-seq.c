...
int main(int argc, char **argv)
{
    ...
    int num_seqque,size_range[5],*ctg_list,*ctg_head,*ctg_loci,*ctg_length,*ctg_index;
    char *pdata;
...
//        printf("fgets command error:\n);
    ...
    i_contig = 0;
    while(fscanf(namef,"%s %d %d %d %s %s",tmptext,&idt,&ctg_loci[i_contig],&ctg_length[i_contig],tmptext,tmptext)!=EOF)
    {
	 ...
	 putc(0+041,fpOutfast);
	    ...
	    putc(seqp->qual[rc]+041,fpOutfast);
         ...
         for(j=0;j<ctg_list[i];j++)
         {
            ...
            seq_st = ctg_loci[idd]-20000;
            seq_ed = ctg_loci[idd]+20000; 
            sprintf(ctgnameout,"%s_%d_%d.fa",seqp->name,seq_st,seq_ed);
/*  input read alignment info line   */
    if((fpOutfast = fopen(ctgnameout,"w")) == NULL)
    {
