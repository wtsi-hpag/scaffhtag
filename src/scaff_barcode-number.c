...
static int *hit_locus,*hit_index,*hit_score,*hit_number,*ctg_index;
...
/*
    ...
    if((hit_number = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_locus2\n");
      exit(1);
    }
...
/*  read the alignment files         */
    ...
    if(IMOD == 0)
    {
      while(fscanf(namef,"%s %s %s %s %s %s %d %s %s",S_Name[i],readname,cc,cc,cc,cc,&hit_number[i],cc,cc)!=EOF)
      {
        int idt;
        st = readname;
    ...
    {
      while(fscanf(namef,"%s %s %s %s %s",S_Name[i],readname,cc,cc,cc)!=EOF)
      {
        int idt;
        st = readname;
        ed= strrchr(readname,'_');
