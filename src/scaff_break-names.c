...
int main(int argc, char **argv)
{
    ...
    fasta *seq;
    int bindex,blocus,blocus2,bgaplen,bscore,setglen;
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_barcode_file> <barcode_sorted_file>\n",argv[0]);
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %s %s",cc,tempc1,cc,S_Name[i])!=EOF)
    {
        i++;
    }
    fclose(namef);
...
/*  read the break file         */
    ...
    while(fscanf(namef,"%s %d %d %d %d %d %d",readname,&bindex,&blocus,&blocus2,&bgaplen,&bscore,&setglen)!=EOF)
    {
      fprintf(namef2,"%s %s %d %d %d %d %d %d\n",readname,S_Name[bindex],bindex,blocus,blocus2,bgaplen,bscore,setglen);
      i++;
    }
    fclose(namef);
    fclose(namef2);
