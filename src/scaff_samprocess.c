...
/*
    ...
    if((map_score = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
    ...
    cigarline=cmatrix(0,2,0,2000000);
    n_readsMaxctg=0;
    n_contig=0;
    n_reads=0;
...
/*  read the alignment files         */
    ...
    n_hits = 0;
    itag = 0;
    while(fscanf(namef,"%s %d %s %d %d",R_Name[i],&itag,temp,&hit_locus1[i],&map_score[i])!=EOF)
    {
        int len1,len2,len,rlen;
        int m_len,d_len,i_len;
        char *st2,*st,*ed,*md,*ed2,*md2;
        char *sts,*eds,*sth,*edh;
        float rate;

//    printf("cigar1: %d %s\n",rlen,cigarline[0]);
    if((itag < 100)&&(map_score[i]>0))
    printf("%s %d %s %d %d\n",R_Name[i],itag,temp,hit_locus1[i],map_score[i]);
        i++;
    }
    fclose(namef);
