...
int main(int argc, char **argv)
{
    ...
    void Barcode_Offset(char **argv,int args,int nSeq);
    ...
    if(argc < 2)
    {
      printf("Usage: %s <input_barcode_file> <barcode_sorted file>\n",argv[0]);
...
/*
    ...
    T_Name=cmatrix(0,nseq,0,Max_N_NameBase);
    M_Name=cmatrix(0,nseq,0,4);
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%s %s %d %s %d",S_Name[i],R_Name[i],&hit_locus[i],M_Name[i],&hit_length[i])!=EOF)
    {
        i++;
    }
    fclose(namef);
    ...
    n_reads=i;
    if(break_flag)
      Barcode_Offset(argv,args,n_reads);
    else
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
        ...
        if((num_hits>=5)) 
        {
          for(n=i;n<j;n++)
          {
             strcpy(T_Name[n-i],R_Name[n]);
             hit_index[n-i] = n-i;
          }
          ArraySort_String(num_hits,T_Name,hit_index);
          for(n=i;n<j;n++)
          {
             int idt = i+hit_index[n-i];
...
{
     int i,j,k,m,n;
     int num_hits,*ctg_offset,*hit_index;
     FILE *namef;
     void ArraySort_String(int n,char **Pair_Name,int *brr);
     void ArraySort_Int2(int n, int *arr, int *brr);
     ...
     for(i=0;i<(nSeq-1);i++)
     {
        ...
        if((num_hits>=1)) 
        {
          for(n=i;n<j;n++)
          {
             ctg_offset[n-i] = hit_locus[n];
             hit_index[n-i] = n-i;
          }
          ArraySort_Int2(num_hits,ctg_offset,hit_index);
          for(n=i;n<j;n++)
          {
             int idt = i+hit_index[n-i];
             fprintf(namef,"%s %s %d %s %d\n",S_Name[n],R_Name[idt],hit_locus[idt],M_Name[idt],hit_length[idt]);
