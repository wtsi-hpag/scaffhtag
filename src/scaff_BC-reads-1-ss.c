...
int main(int argc, char **argv)
{
    ...
    char barcode[500],rdname[500],barcoderc[100],barcode2[100];

    seq=NULL;
    if(argc < 2)
    {
      printf("Usage: %s <input read_1 file <output read_1 file> <output name_1 file> \n",argv[0]);
      exit(1);
    }
...
//        printf("align0 %d %s\n",nPair,rdname);
           ...
           else if(i==2)
           {
             ...
             st = base0+5;
             len = strlen(base0);
             strncpy(barcode,st,13);
             st = base0+18;
             strncpy(barcode2,st,13);
             for(j=13;j>=0;j++)
             {
                if(barcode2[j-1] == 'A')
                  barcoderc[13-j] = 'T';
        ...
        printf("align00 %d %s %s %s\n",num_align,rdname,barcode,barcode2,barcoderc);
           }
        }
      }
      num_align++;
